...
export default function AsciiConverter() {
  ...
  const loadDefaultImage = () => {
    ...
    // Cache-bust the local default image so replacements in /public are reflected immediately.
    img.src = `${DEFAULT_IMAGE_SRC}?v=${Date.now()}`
  }
  ...
  const convertToAscii = () => {
    try {
      ...
      const width = Math.max(1, Math.floor(img.width * resolution))
      const height = Math.max(1, Math.floor(img.height * resolution))
      ...
      for (let y = 0; y < img.height; y += heightStep) {
        ...
        for (let x = 0; x < img.width; x += widthStep) {
          ...
          if (!grayscale) {
            ...
            coloredRow.push({ char, color: asciiColor })
          }
        }
  ...
  const downloadAsciiImage = () => {
    if (!asciiArt || coloredAsciiArt.length === 0) {
      setError("No ASCII image to download")
      return
    }
        ...
        >
          <div className="space-y-4 p-2 md:p-4 border border-stone-700 rounded-md">
            <div className="space-y-1">
              <h1 className="text-lg text-stone-100 font-bold">Image to ASCII Art</h1>
            ...
            <div className="space-y-4 pt-2">
              ...
              <div className="space-y-2 border-t border-stone-700 pt-4">
                ...
                <Select value={charSet} onValueChange={setCharSet}>
                  ...
                  <SelectContent className="bg-stone-800 border-stone-700 text-stone-300">
                    ...
                    <SelectItem value="binary" className="focus:bg-stone-700 focus:text-stone-100">
                      Binary 0/1
                    </SelectItem>
              ...
              <div className="space-y-2 border-t border-stone-700 pt-4">
                <div className="flex items-center justify-between gap-3">
                  <Label htmlFor="ascii-color" className="text-stone-300">
                    ASCII Color
                  </Label>
                  <div className="flex items-center gap-2">
                    <span className="text-xs uppercase text-stone-500">{asciiColor}</span>
                    <input
                      id="ascii-color"
                      type="color"
                      value={asciiColor}
                      onInput={(event) => setAsciiColor(event.currentTarget.value)}
                      onChange={(event) => setAsciiColor(event.target.value)}
                      className="h-9 w-12 cursor-pointer rounded-md border border-stone-700 bg-stone-800 p-1"
                      title="Change monochrome ASCII color"
                    />
                  </div>
                </div>
              ...
              <div className="flex gap-2 pt-4 border-t border-stone-700">
                ...
                <Button
                  ...
                  title="Download ASCII text"
                  aria-label="Download ASCII text"
